/*
* NOTE:
* 1.window.onload必须等到页面内包括图片的所有元素加载完毕后才能执行。而$(document).ready()是DOM结构绘制完毕后就执行，不必等到加载完毕。
...
* 4.Jquery对象中如果dom数组存在多个，调用attr()或者find等只会对第一个dom对象调用；
* */
...
(function ($) {
    ...
    $(function () {
        ...
        /*在支付界面中实现点击不同的按钮显示不同的支付图片（注意.bind而不能直接.click）*/
...
//点赞+1，每次点赞将likeId置为该文章的id，阻止在一次点赞中重复点赞，bug是点击赞了其它文章后覆盖了likeId则可以为再次赞该文章，应该设置一个用户只能对一篇文章点赞一次？
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $.cookie(
                    "likeId",
                    articleId,//需要cookie写入的业务
                    /*此处设置能读取当前网页的cookie的顶级目录为/,路径与cookie的键共同作为cookie的标识*/
                    {
                        "path": "/", //cookie的默认属性
                    }
                );
